    ...
    try:
        ...
        if response_semanal.rows:
            for row in response_semanal.rows:
                ...
                nice_date = f"{raw_date[6:8]}/{raw_date[4:6]}" if len(raw_date) >= 8 else raw_date
                temp_semanal.append({
                    "raw": raw_date, 
                    "fecha": nice_date, 
        ...
        grafica_final = [{"fecha": d["fecha"], "usuarios": d["usuarios"]} for d in temp_semanal]
        ...
        total_val = int(response_total.rows[0].metric_values[0].value) if response_total.rows else 0
        ...
        return {
            "grafica_semanal": grafica_final if grafica_final else [{"fecha": "Sin datos", "usuarios": 0}],
...
def get_conversion_data():
    ...
    try:
        ...
        return conversion_stats if conversion_stats else [{"nombre": "Visitas", "valor": 0}, {"nombre": "Ventas", "valor": 0}]
    except Exception as e:
