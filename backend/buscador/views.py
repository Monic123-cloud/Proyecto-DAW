...
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def mis_datos_cliente(request):
    """
    Devuelve los datos del cliente logueado para mostrarlos en checkout.
    No permite modificarlos desde checkout.
    """

    user = request.user

    def leer_campo(*nombres):
        for nombre in nombres:
            valor = getattr(user, nombre, None)
    ...
    if not nombre:
        nombre = leer_campo("nombre", "name", "first_name")

    if not nombre:
        nombre = leer_campo("username")

    data = {
        "id_usuario": getattr(user, "id", None),
        "nombre": nombre,
        "email": leer_campo("email", "correo") or leer_campo("username"),
        "telefono": leer_campo("telefono", "phone", "movil", "mobile"),
        "direccion": leer_campo("direccion", "address"),
        "municipio": leer_campo("municipio", "localidad", "city"),
        "cp": leer_campo("cp", "codigo_postal", "postal_code"),
    }
