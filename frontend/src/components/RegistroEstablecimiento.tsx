...
/**
 * IMPORTANTÍSIMO:
 * - El orden de "libraries" debe ser EXACTAMENTE el mismo en TODA la app,
 *   o el loader peta con: "Loader must not be called again with different options"
 */
...
const ESTRUCTURA = {
  "Educación y Cultura": {
    ...
    Instituto: ["Ciclos Formativos", "Bachillerato", "Educación de Adultos", "Otros..."],
    Guardería: null,
  ...
  "Salud y Belleza": {
    ...
    Peluquería: null,
    Manicura: null,
    "Gimnasio / Centro Deportivo": ["Gym", "Yoga", "Zumba", "Baile", "Boxeo", "Otros..."],
    Fisioterapia: null,
    Óptica: null,
    Veterinaria: null,
  ...
  "Hostelería (Ocio)": {
    "Bar / Cafetería": null,
    Restaurante: ["Pizzería", "Hamburguesería", "Comida Española", "Mexicano", "Sushi", "Oriental", "Otros..."],
    "Pub / Discoteca": null,
...
export default function RegistroEstablecimiento() {
  // ✅ Hook 1: loader SIEMPRE se llama
  ...
  // ✅ A partir de aquí: TODOS los hooks antes de cualquier return
  const [vista, setVista] = useState<"seleccion" | "busqueda" | "formulario">("seleccion");
  const [editId, setEditId] = useState<number | null>(null);
  const [cifBusqueda, setCifBusqueda] = useState("");
  const [loading, setLoading] = useState(false);
  const [password, setPassword] = useState("");

  // Evitamos tipar con google.maps... antes de que cargue el script
  ...
  const buscarMiNegocio = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const catExiste =
          ESTRUCTURA[grupoKey] && Object.keys(ESTRUCTURA[grupoKey] as any).includes(data.categoria);

        setFormData({
          ...data,
          categoria: catExiste ? data.categoria : data.categoria ? "Otros..." : "",
  ...
  const eliminarNegocio = async () => {
    if (!window.confirm("¿Estás seguro de que quieres borrar este negocio?")) return;
  ...
  const onPlaceChanged = () => {
    ...
    const addressInfo = { direccion: place.name || "", cp: "", municipio: "", provincia: "" };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editId && !authService.isAuthenticated()) {
      alert("Sesión expirada. Busca tu negocio de nuevo.");
      setLoading(false);
      return;
    }
    ...
    if (password) datosAEnviar.password = password;
    else if (!editId) {
    ...
    const url = editId ? `${ENDPOINTS.ESTABLECIMIENTOS}${editId}/` : ENDPOINTS.ESTABLECIMIENTOS;
    ...
    try {
      ...
      if (response.ok) {
        alert(editId ? "¡Cambios guardados correctamente!" : "¡Negocio registrado con éxito!");
        setVista("seleccion");
      } else {
  ...
  // ✅ Render condicional (NO hooks aquí)
  if (loadError) return <div style={{ color: "white", padding: 20 }}>Error al cargar Google Maps</div>;
  if (!isLoaded) return <div style={{ color: "white", padding: 20 }}>Cargando mapa…</div>;
  ...
  return (
    ...
    >
      ...
      <div className="text-center mb-4">
        ...
        >
          <img src="/images/Close-logo_1.png" alt="logo" style={{ width: "100%", height: "100%", objectFit: "contain" }} />
        </div>
      ...
      <div className="mx-auto w-100" style={{ maxWidth: "450px" }}>
        ...
        {vista === "busqueda" && (
          <div className="bg-dark p-4 rounded-4 shadow border border-warning">
            ...
            <p className="text-white-50 small">Introduce tu CIF/NIF para editar tus datos:</p>
        ...
        {vista === "formulario" && (
          ...
          >
            <h4 className="text-white mb-4 text-center">{editId ? "Editar Negocio" : "Nuevo Registro"}</h4>

            {/* TIPO */}
            <div className="mb-3">
              <label className="form-label text-white-50 small fw-bold">TIPO DE NEGOCIO</label>
                ...
                {["Comercio", "Productor Local"].map((t) => (
                  <button
                    ...
                    className={`btn btn-sm w-50 ${
                      formData.tipo_negocio === t ? "btn-warning" : "btn-outline-secondary text-white"
                    }`}
                    onClick={() => setFormData({ ...formData, tipo_negocio: t })}
                  >
                    {t}
                  </button>
                ))}
            ...
            {/* NOMBRE */}
            <div className="mb-3">
              <label className="form-label text-white fw-bold small">Nombre del Negocio</label>
              <input
                type="text"
                className={inputClasses}
            ...
            {/* CIF */}
            <div className="mb-3">
              <label className="form-label text-white fw-bold small">CIF / NIF *</label>
              <input
                type="text"
                className={`${inputClasses} ${
                  formData.cif_nif && !validarDocumentoCompleto(formData.cif_nif) ? "is-invalid" : ""
                }`}
                value={formData.cif_nif}
                onChange={(e) =>
                  setFormData({ ...formData, cif_nif: e.target.value.toUpperCase().trim() })
                }
                required
                placeholder="Ej: 12345678Z o B12345678"
              />
              {formData.cif_nif && !validarDocumentoCompleto(formData.cif_nif) && (
                <div className="invalid-feedback" style={{ display: "block" }}>
                  El número de identificación no es válido (letra de control o formato incorrecto).
                </div>
              )}
            </div>
            ...
            {/* GRUPO */}
            <div className="mb-3">
              <label className="form-label text-white fw-bold text-uppercase" style={{ fontSize: "0.75rem", letterSpacing: "1px" }}>
              ...
              <select
                ...
                onChange={(e) =>
                  setFormData({ ...formData, grupo: e.target.value, categoria: "", subcategoria: "" })
                }
              >
            ...
            {formData.grupo && (
              <div className="mb-3">
                <label className="form-label text-white fw-bold text-uppercase" style={{ fontSize: "0.75rem" }}>
                  2. Categoría
                </label>
                <select
                  className={selectClasses}
                ...
                >
                  ...
                  {Object.keys((ESTRUCTURA as any)[formData.grupo] || {}).map((c: string) => (
            ...
            {/* SUBCATEGORIA */}
            {formData.categoria &&
              Array.isArray(((ESTRUCTURA as any)[formData.grupo] || {})[formData.categoria]) && (
                <div className="mb-3">
                  <label className="form-label text-warning fw-bold text-uppercase" style={{ fontSize: "0.75rem" }}>
                  ...
                  >
                    ...
                    {(((ESTRUCTURA as any)[formData.grupo] || {})[formData.categoria] || []).map((s: string) => (
            ...
            <div className="mb-3">
              <label className="form-label text-white fw-bold small">Busca tu Dirección *</label>
            ...
            {/* NUMERO */}
            <div className="mb-3">
              <label className="form-label text-white fw-bold small">Número / Portal *</label>
              <input
                type="text"
                className={inputClasses}
            ...
            <div className="row mb-3 g-2">
              <div className="col-8">
                <label className="form-label text-white-50 small">Municipio</label>
                <input type="text" className={inputClasses} value={formData.municipio} readOnly />
              </div>
              <div className="col-4">
              ...
              <div className="col-12">
                <label className="form-label text-white-50 small">Provincia</label>
                <input type="text" className={inputClasses} value={formData.provincia} readOnly />
              </div>
            </div>

            {/* CONTACTO */}
            <div className="mb-3">
              <label className="form-label text-white fw-bold small">Correo electrónico</label>
              <input
                type="email"
                className={inputClasses}
            ...
            <div className="mb-3">
              <label className="form-label text-white fw-bold small">Crea una contraseña para gestionar tu negocio:</label>
              <input
                type="password"
                className={inputClasses}
                value={password}
            ...
            <div className="mb-3">
              <label className="form-label text-white fw-bold small">Teléfono de contacto *</label>
              <input
                type="tel"
                className={inputClasses}
            ...
            <div className="d-grid gap-2 mt-4">
              <button
                type="submit"
                disabled={loading || !validarDocumentoCompleto(formData.cif_nif) || !validarCP(formData.cp)}
                className="btn btn-warning fw-bold py-3 text-uppercase shadow-sm w-100 mb-3"
                style={{
                  color: "#275656",
                  cursor:
                    loading || !validarDocumentoCompleto(formData.cif_nif) || !validarCP(formData.cp)
                      ? "not-allowed"
                      : "pointer",
                  opacity:
                    loading || !validarDocumentoCompleto(formData.cif_nif) || !validarCP(formData.cp) ? 0.6 : 1,
                }}
              >
                {loading ? "Procesando..." : editId ? "Guardar Cambios" : "Finalizar Registro"}
              </button>

              <div className="d-flex justify-content-between align-items-center">
                {editId && (
                  <button type="button" className="btn btn-outline-danger btn-sm border-0" onClick={eliminarNegocio}>
